...
> *Satu link untuk semua bisnismu — link-in-bio + toko/katalog + checkout + dashboard seller + Admin Master.*
...
[![Payment: QRIS](https://img.shields.io/badge/payment-QRIS%20(BuatQris)-red)](https://api.buatqris.site)
...
- 🛍️ **Link-in-bio + toko** dalam satu halaman — produk, link, sosial media, galeri
- 💳 **Checkout otomatis QRIS** via BuatQris — webhook HMAC SHA256, status order real-time
- 👑 **Dashboard seller + Admin Master** — pesanan, tema, approval Premium, ledger keuangan, penarikan
- 🔔 **Push notification** Firebase Cloud Messaging via `pg_net` trigger
- 📱 **Mobile-first** — untuk UMKM, seller online, creator, dan pengguna HP-first
- 🛡️ **Security-first** — RLS semua tabel, anti-XSS, HMAC webhook, rate limit, CSP ketat
- 💻 **Dibuat dari HP** — SPCK Editor + Termux, tanpa laptop
