(function () {
  ...
  async function saveSettings(settings) {
    const payload = {
      ...
      maintenance_pages: Array.isArray(settings.maintenance_pages) ? settings.maintenance_pages : [],
  ...
  function localCreateNotification(row) {
    ...
    const notification = {
      ...
      link_url: isSafeRelativeHref(row.link_url || '') ? String(row.link_url).trim() : normalizeExternalUrl(row.link_url || '/seller/notifications', '/seller/notifications'),
  ...
  async function adminReviewPremiumRequest(requestId, action = 'approved', days = 30) {
    ...
    localCreateNotification({
      ...
      link_url: action === 'approved' ? '/seller/dashboard' : '/seller/upgrade',
