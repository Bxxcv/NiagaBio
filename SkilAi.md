...
## 10. STORE CSS RESPONSIVE GUARDRAILS — 2026-08-20

`assets/css/v2/store.css` adalah **store-specific layer** dan saat ini menjadi layer responsive untuk tema `service`.

### Wajib
- Semua selector visual baru di `store.css` harus scoped ke `.public-theme-service`.
- Jangan membuat selector global seperti `.public-card`, `.public-link`, `.public-product-card`, `.public-body`, `.public-footer`, atau `.public-page` di `store.css` karena tema lain masih berasal dari `main.css`.
- `main.css` tetap dimuat sebelum `foundation.css` lalu `store.css` pada `u.html`.
- Mobile dan desktop **harus memiliki layout yang berbeda secara sengaja**, bukan sekadar memperbesar container.
- Breakpoint target: HP kecil `<360px`, mobile `360–767px`, tablet `768–991px`, desktop `≥992px`, dan desktop lebar `≥1200px`.
- Produk: 1 kolom pada HP sangat sempit, 2 kolom pada mobile, 3 kolom pada tablet/desktop, 4 kolom pada desktop lebar bila ruang cukup.
- Desktop `service` boleh menggunakan layout dua kolom (hero + body) untuk memanfaatkan lebar layar; mobile tetap hero di atas body.
- Jangan mengubah `public-page.js`, RPC, auth, routing, atau data flow untuk menyelesaikan masalah CSS.

### Regression guard
Sebelum deploy perubahan `store.css`, uji minimal:
1. `service` di HP kecil.
2. `service` di HP besar.
3. `service` di tablet.
4. `service` di desktop.
5. satu tema lama (`minimal`) untuk memastikan `main.css` tidak tertimpa.
6. satu tema premium (`fashion` atau `gadget`) untuk memastikan karakter/desktop layout tidak rusak.
