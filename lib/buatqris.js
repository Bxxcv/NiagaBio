...
function verifyOrderAccessToken(orderId, token) {
  const secret = env('ORDER_ACCESS_SECRET', '');
  if (!secret) {
    // Fail-closed: if secret missing, explicitly deny verification.
    // Log a deploy-time alert without revealing the secret value.
    try {
      console.error('[NiagaBio] [ALERT] ORDER_ACCESS_SECRET not configured — denying order access token verification');
    } catch (_) {}
    return false;
  }
