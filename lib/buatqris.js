...
function verifyOrderAccessToken(orderId, token) {
  const secret = env('ORDER_ACCESS_SECRET', '');
  if (!secret) {
    // Fail-closed: ORDER_ACCESS_SECRET sudah dikonfirmasi aktif di production
    // (dites Sep 2026). Kalau tiba-tiba kosong (redeploy/environment baru
    // yang lupa di-set), tolak akses daripada diam-diam membuka IDOR lagi.
    console.error('[NiagaBio] [ALERT] ORDER_ACCESS_SECRET tidak dikonfigurasi — menolak verifikasi order access token');
    return false;
  }
